...
// NOTE: SYSTEM_PROMPT aur [Subject: X] extraction logic ab
// agentic-service/app/chains/doubt_chain.py mein hai — yahan se hata diya.
...
const askDoubt = async (requester, { question, subject }, imageFile) => {
  ...
  let result;
  try {
    result = await agenticService.askDoubt({ question: trimmedQuestion, subject, imageUrl,  instituteId: requester.instituteId, });
   } catch (err) {
    logger.error(`AGENTIC SERVICE ERROR: ${err.response?.data ? JSON.stringify(err.response.data) : err.message}`);
    throw new ApiError(503, 'AI tutor is a bit busy right now. Please try again in a moment.');
  }
