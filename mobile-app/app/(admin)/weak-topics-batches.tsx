...
export default function WeakTopicsBatchesScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <Text style={[typography.caption, { color: colors.textMuted, padding: spacing.lg, paddingBottom: 0 }]}>
        Select a batch to see its weak topics
      </Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <PressableCard
            style={styles.row}
            onPress={() =>
              router.push({ pathname: '/(admin)/batch-weak-topics', params: { batchId: item._id, batchName: item.name } })
            }
          >
            <View style={{ flex: 1 }}>
