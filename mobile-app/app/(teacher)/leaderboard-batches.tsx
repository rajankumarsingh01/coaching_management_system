...
export default function TeacherLeaderboardBatchesScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <Text style={[typography.caption, { color: colors.textMuted, padding: spacing.lg, paddingBottom: 0 }]}>
        Select a batch to view its test leaderboard
      </Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <PressableCard
            style={styles.row}
            onPress={() =>
              router.push({ pathname: '/(teacher)/leaderboard', params: { batchId: item._id, batchName: item.name } })
            }
          >
            <View style={{ flex: 1 }}>
